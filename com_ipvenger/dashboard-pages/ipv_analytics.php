...
defined('_JEXEC') or die;
    ...
    $base_url = JURI::base();
    $ipcc_url = $base_url .  'index.php?option=com_ipvenger&view=ipv_ipcc';
    $adv_url  = $base_url .  'index.php?option=com_ipvenger&view=ipv_advanced';

	$clr_green 	= ipv_clr_range_green;
	$clr_yellow	= ipv_clr_range_yellow;
	$clr_orange	= ipv_clr_range_orange;
	$clr_red 	= ipv_clr_range_red;

	$ip_bl_color = ipv_clr_ip_blacklist;
	$cn_bl_color = ipv_clr_ctry_blacklist;
	$ipq_color 	 = ipv_clr_ipq_block;
	$allow_color = ipv_clr_allow;
		
	$clr_country_pct = ipv_clr_country_pct_block;
	$clr_country_avg = ipv_clr_country_avg_ipq;

	ipv_get_summary_data( array(7,14,30), $total_blocked, $total_requests ); 

	$total_blocked  = json_encode( $total_blocked );
	$total_requests = json_encode( $total_requests );

	$rules_array = ipv_get_rules_array( 'blacklist', 'country' );
	...
	$blacklisted_countries = json_encode( $blacklisted_countries );
...
<div class="ipv" id="ipv-country-dialog" style="display:none">
	...
	<div id="ipv-country-detail-pie"
			style="background: url( '${GLOBALS['ipv_image_url']}
...
<script type="text/javascript">
	...
	function display_pacifiers() {
		...
		jQuery( "#ipv-country-bar" ).html( 
			...
			'${GLOBALS['ipv_image_url']}/spin-white.gif" alt="Loading...">'
		);
	...
	var ip_bl_color = "$ip_bl_color";
	var cn_bl_color = "$cn_bl_color";
	var ipq_color 	= "$ipq_color";
	var allow_color = "$allow_color";
	...
	pie_bg_url = '${GLOBALS['ipv_image_url']}/pie_bg.png'; 
	pie_nd_url = '${GLOBALS['ipv_image_url']}/pie_bg_nd.png';
				...
				if ( data["disp_data"].length > 0 ) {
					...
					if ( data["cat_data"].length > 0 ) {
						...
						jQuery('#ipv-country-detail-pie').css(
							...
							'${GLOBALS['ipv_image_url']}/ctry_pie_bg.png )');
...
<div class="analytics ipvmain">
    ...
    <img src= "${GLOBALS['ipv_image_url']}/hdr-text/analytics.png">
	</div>
			...
			<div id="disp-pie-container">
				...
				<div id="disp-pie-text">
					<div class="pie-text-header">
						<img alt="?" src=
							"${GLOBALS['ipv_image_url']}/tooltip_question.png" 
...
score.  It shows the main reason that the IPQ score was too high.
				...
				<div>
					...
					<div id="ipv-daily-container">
						...
						<div class="legend">
							<div class="legend-color">
							<div class="legend-color-inner"
								style="background:$ipq_color">&nbsp;</div>
							</div>
							<label>Failed IPQ Score</label>
							<div class="legend-color">
							<div class="legend-color-inner"
								style="background:$cn_bl_color">
								&nbsp;
							</div>
							</div>
			...
			<div id="ipv-country-bar-container">
				...
				<div id="ipv-country-sort-container">
					...
					<span id="ipv-country-sort-block" 
						style="font-weight:bold;color:$clr_country_pct"
						onclick="ipv_sort_country_bar( 'block' )">
							% Blocked by Country
					</span>
					|
					<span id="ipv-country-sort-ipq" 
						style="color:$clr_country_avg"
		...
		<div class="analytics-pane hide_if_empty">
			...
			<div id="ipv-country-list-link-container" style="clear:both">
				<a class=ipv-link-button href="$adv_url">
	...
	<div class="ip-ticker">
		...
		<div class="ipv-ticker-bookend" id="ipv-ticker-footer">
			<a href="$ipcc_url"> View all blocks </a>
		</div>
	</div>
