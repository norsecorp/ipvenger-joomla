...
function ipv_gen_captcha() {
	...
	$img = new IPVSecurimage();

	//$img->ttf_file        = './Quiff.ttf';
	//$img->captcha_type    = IPVSecurimage::SI_CAPTCHA_MATHEMATIC; // show a simple math problem instead of text
	...
	$img->image_bg_color  = new IPVSecurimage_Color("#CCCCCC");   // image background color
	$img->text_color      = new IPVSecurimage_Color("#333333");   // captcha text color
	//$img->num_lines       = 4;                                 // how many lines to draw over the image
	$img->line_color      = new IPVSecurimage_Color("#333333");   // color of lines over the image
	//$img->image_type      = SI_IMAGE_JPEG;                     // render as a jpeg image
	//$img->signature_color = new IPVSecurimage_Color(rand(0, 64),
	//                                            rand(64, 128),
...
function ipv_insert_captcha( $image_data, $response ) {
	...
	$image_data = ipv_db_escape_string( $image_data );
