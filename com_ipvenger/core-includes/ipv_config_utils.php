...
/**
 * function ipv_update_site_ipq the ipq score for given site type
 *
...
function ipv_update_site_ipq( $site, $ipq ) {
    ...
    $ipq = ipv_db_escape_string( $ipq );
    $site = ipv_db_escape_string( $site );
	...
	$short_name, &$display_name, &$ipq_level, &$ipq_level_desc, &$desc_text  )
{
	...
	$short_name = ipv_db_escape_string( $short_name );
...
function ipv_set_site_type( $site_type ) {
    ...
    $site_type = ipv_db_escape_string( $site_type );
...
function ipv_set_default_risk( $risk ) {
    ...
    $risk = ipv_db_escape_string( $risk );
...
function ipv_get_rules_array( $rule_type, $field ) { 
    ...
    $field      = ipv_db_escape_string( $field );
...
function ipv_add_exception( $field_type, $mask, $mask_type, $action ) {
		...
		$table		= IPV_EXCEPTION;
        $mask       = ipv_db_escape_string( $mask );
        $mask_type  = ipv_db_escape_string( $mask_type );
        $field_type = ipv_db_escape_string( $field_type );
        $action     = ipv_db_escape_string( $action );
...
function ipv_delete_exception( $type, $mask ) {
        ...
        $mask  = ipv_db_escape_string( $mask );
        $type  = ipv_db_escape_string( $type );
...
/**
 * function to put basic site information into the database so it can 
 * be accessed by routines (especially block_message()) that operate
 * outside of the cms loop
 */
	...
	$ipcc, $stylesheet, $logo, $name, $description, $block_path ) 
{
    ...
    $ipcc        = ipv_db_escape_string( $ipcc );
    $stylesheet  = ipv_db_escape_string( $stylesheet );
    $logo        = ipv_db_escape_string( $logo );
    $name        = ipv_db_escape_string( $name );
    $description = ipv_db_escape_string( $description );
    $block_path  = ipv_db_escape_string( $block_path );
...
function ipv_set_default_email( $admin_email ) {
    ...
    $admin_email = ipv_db_escape_string( $admin_email );
...
function ipv_matches_excp( $action, $excp_type, $mask ) {
    ...
    $excp_type  = ipv_db_escape_string( $excp_type );
    $mask       = ipv_db_escape_string( $mask );
    $action     = ipv_db_escape_string( $action );
	...
	if ( ( $q_result ) && ipv_db_fetch_assoc( $q_result ) ) return true;
